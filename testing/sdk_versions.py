...
def upgrade(
        ...
        timeout_seconds=install.DEFAULT_TIMEOUT_SECONDS):
    ...
    upgrade_or_downgrade(test_package_name, test_package_name, running_task_count, additional_options, timeout_seconds)
...
def downgrade(
    ...
    upgrade_or_downgrade(universe_package_name, test_package_name, running_task_count, additional_options, timeout_seconds)
...
def soak_upgrade_downgrade(
    ...
    upgrade_or_downgrade(test_package_name, service_name, running_task_count, install_options, timeout_seconds, 'stub-universe')

    print('Downgrading to Universe version')
    # Default Universe is at --index=0
    upgrade_or_downgrade(universe_package_name, service_name, running_task_count, install_options, timeout_seconds)
...
def upgrade_or_downgrade(
    ...
    plan.wait_for_completed_deployment(service_name, timeout_seconds)
    sdk_utils.out('Checking that all tasks have restarted')
    tasks.check_tasks_updated(service_name, '', task_ids, timeout_seconds)

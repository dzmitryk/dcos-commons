...
def setup_module(module):
    ...
    sdk_install.install(
        ...
        additional_options=sdk_sdk_networks.ENABLE_VIRTUAL_NETWORKS_OPTIONS)
...
def test_service_overlay_health():
    ...
    for task in broker_tasks:
        sdk_sdk_networks.check_task_network(task)
...
def test_overlay_network_deployment_and_endpoints():
    ...
    endpoints = sdk_sdk_networks.get_and_test_endpoints("", PACKAGE_NAME, 2)
    ...
    broker_endpoints = sdk_sdk_networks.get_and_test_endpoints("broker", PACKAGE_NAME, 4)
    sdk_sdk_networks.check_endpoints_on_overlay(broker_endpoints)

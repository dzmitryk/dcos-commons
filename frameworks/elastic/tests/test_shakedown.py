...
def test_endpoints():
    ...
    for nodetype in ('coordinator', 'data', 'ingest', 'master'):
        ...
        assert endpoints['dns'][0].startswith(sdk_sdk_hosts.autoip_host(FOLDERED_SERVICE_NAME, nodetype + '-0-node'))
        assert endpoints['vips'][0].startswith(sdk_sdk_hosts.vip_host(FOLDERED_SERVICE_NAME, nodetype))
...
def test_xpack_toggle_with_kibana(default_populated_index):
    ...
    shakedown.install_package("kibana", options_json={
        "kibana": {
            "elasticsearch_url": "http://" + sdk_sdk_hosts.vip_host(FOLDERED_SERVICE_NAME, "coordinator", 9200)
        }})
...
def test_losing_and_regaining_index_health(default_populated_index):
    ...
    shakedown.kill_process_on_host(sdk_sdk_hosts.system_host(FOLDERED_SERVICE_NAME, "data-0-node"), "data__.*Elasticsearch")
...
def test_master_reelection():
    ...
    shakedown.kill_process_on_host(sdk_sdk_hosts.system_host(FOLDERED_SERVICE_NAME, initial_master), "master__.*Elasticsearch")
    wait_for_expected_nodes_to_exist(service_name=FOLDERED_SERVICE_NAME)
